# My Work — Take-Home Assignment: The Untested API
...
└── task-api/
    ...
    │   ├── swagger.js              # ← OpenAPI 3.0 spec (new)
    ...
    ├── tests/
    │   ├── taskService.test.js     # ← Unit tests I wrote (37 tests)
    │   └── tasks.routes.test.js    # ← Integration tests I wrote (25 tests)
    └── demo.js                     # ← Live API demo script (new)
```
...
`GET /tasks?status=blah` returns `200 []` — indistinguishable from "no matching tasks." A client with a typo in their status filter would never know. I documented this but chose not to fix it as it's outside the bug-fix scope.
...
**URL:** `http://localhost:3000/api-docs` (start the server first)
...
### Files added for Swagger

| File | Purpose |
|---|---|
| `src/swagger.js` | Full OpenAPI 3.0 spec (all endpoints, schemas, examples) |
| `src/app.js` | Mounted `swagger-ui-express` at `/api-docs` |
